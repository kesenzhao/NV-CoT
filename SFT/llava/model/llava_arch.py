...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # print(input_ids,labels, images.shape)
        # de
    ...
    def prepare_cot_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                new_labels.append(labels[batch_idx])
                cur_image_idx += 1
                cur_token_types = torch.cat([torch.zeros(cur_input_embeds_1.shape[0], dtype=torch.long), torch.zeros(cur_image_features[0:0].shape[0], dtype=torch.long)], dim=0)
                torch.zeros(cur_input_embeds.shape[0], dtype=torch.long)
                token_types.append(cur_token_types)
                continue
            ...
            for i in range(num_images + 1):
                ...
                cur_token_types.append(torch.zeros(cur_len, dtype=torch.long))
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    ...
                    cur_token_types.append(torch.ones(cur_len, dtype=torch.long))

            cur_new_input_embeds = [x.to(self.device) for x in cur_new_input_embeds]
            cur_token_types = [x.to(self.device) for x in cur_token_types]

            cur_new_input_embeds = torch.cat(cur_new_input_embeds)
            cur_token_types = torch.cat(cur_token_types)
        ...
        if tokenizer_model_max_length is not None:
            ...
            token_types = [x[:tokenizer_model_max_length] for x in token_types]
        ...
        for i, (cur_new_embed, cur_new_label, cur_token_types) in enumerate(zip(new_input_embeds, new_labels, token_types)):
            ...
            if getattr(self.config, 'tokenizer_padding_side', 'right') == "left":
                ...
                token_types_padded.append(torch.cat((
                    torch.full((max_len - cur_len,), -1, dtype=cur_token_types.dtype, device=cur_token_types.device),
                    cur_token_types
                ), dim=0))
                if cur_len > 0:
        ...
        token_types = torch.stack(token_types_padded, dim=0)
